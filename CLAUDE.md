...
Deleting a specimen row is never just a `DELETE`; the `DELETION` section of `data_manager.py` owns all of it, and new delete paths belong there rather than calling `.delete()` directly. Three things travel with a row: its **photos** (in the `specimen-photos` bucket, not on the row — dropping the row alone orphans objects that keep counting against storage and stay reachable by URL), its **vialed-out children** (`parent_specimen_id` is `on delete set null`, so a plain delete detaches them into individuals whose collection event no longer exists instead of removing them), and its batch's **`vialed_out` tally** (deleting one child decrements it, returning that specimen to the batch — otherwise a real caught mosquito leaves every total). `tests/test_deletion.py` pins the conservation property in both directions; `delete_specimen_records()` returns a summary whose `tally_failures`/`not_deleted`/`photos_orphaned` keys must be surfaced, not swallowed. The genus a child was vialed as is read **only** via `subsampled_genus_of()` — an identification overwrites the child's screening result, so it survives under `subsampled_genus`, not in the current result. User-facing docs: `docs/DELETING_ENTRIES.md`.

> **Deletion is ownership-scoped: you may delete a row you recorded, or any row if you are a registered admin.** The boundary is the database — `sql/add_ownership_delete_policies.sql` replaces the earlier blanket `using (true)` DELETE policies with `collector_id = auth.uid()::text or public.is_app_admin()` (and `submitted_by` on the two side tables). Admins are rows in `public.app_admins`, which has **no** write policy: they are added in the SQL editor with the service role, so a compromised session cannot promote itself. `data_manager.py::_partition_by_ownership()` applies the same rule in Python *before* the delete — not as the enforcement (RLS would refuse it regardless) but so the refusal can be **explained**: RLS declines by matching zero rows, which is indistinguishable from "already deleted". It reports them under the summary's `refused_not_yours`, which must be surfaced like the other failure keys. `delete_all_specimen_records()` with no `collector_id`, and both `_delete_all_rows()` side-table resets, are admin-only — they have no scoped form. The project-wide delete additionally asks for `ADMIN_DELETE_PASSKEY_HASH` (`utils/auth.py::verify_admin_passkey`, fails closed when unset); that passkey is a confirmation step against an unattended session, **not** the security boundary — don't let it become the only check in front of a destructive path. There is exactly **one** project-wide delete, in `components/profile.py::_render_trial_data_reset()`; the Site Log page's duplicate was removed, since two controls for the same irreversible action had drifted to different gates (the Site Log one asked for the passkey, Profile's "Every record in the project" scope did not). Keep it that way — a new "clear everything" button belongs in that function, behind the same passkey. Scoped resets deliberately don't ask for it. See `docs/DELETING_ENTRIES.md`.
...
> **Storage is the same, and it is a separate policy surface** (`sql/add_storage_policies.sql` established the bucket's INSERT/SELECT and `add_delete_policies.sql` its DELETE; `sql/add_photo_ownership.sql` now supersedes the INSERT and DELETE of both). A blocked object delete returns **200 with an empty list**, not an error, so `delete_specimen_photos()` counts the objects the bucket sends back and reports the rest as `photos_orphaned`.

> **Photo objects are ownership-scoped by their path.** `_upload_photo_bytes()` writes `{collector_id}/{specimen_id}/{uuid}.ext`, and the bucket's policies read `(storage.foldername(name))[1]` as the owner — INSERT confined to your own prefix, DELETE `= auth.uid() or public.is_app_admin()`, SELECT still public. The leading segment exists because `storage.objects` has no foreign key back to `specimen_records` and the row is deleted *before* its photos, so a policy joining back to find the owner would find the row already gone and refuse every legitimate delete; the path makes the check local to the object. **Don't change that path shape** — a first segment that isn't a uid matches no user, and the policy silently stops granting anything (`tests/test_deletion.py::TestPhotoObjectsCarryTheirOwner` pins it, including that the returned public URL round-trips back to the uploaded path). Objects predating this sit at `{specimen_id}/{uuid}.ext`, are admin-only to delete, and are moved by `scripts/migrate_photo_paths.py` (service role; run it *before* the SQL). `_storage_path_from_public_url()` reads both shapes. A blocked *upload* is the exception that does raise — 403 "new row violates row-level security policy" — which is what an upload arriving as `anon` gets. That is why `utils/auth.py::_apply_storage_token()` writes to storage3's `_headers` and not the httpx session's: storage3 passes `_headers` to httpx explicitly per request, and an explicit header beats a session default, so setting the session alone is silently ignored.
