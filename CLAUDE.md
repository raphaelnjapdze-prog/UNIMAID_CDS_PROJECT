...
> **Storage is the same, and it is a separate policy surface** (`sql/add_storage_policies.sql` for the bucket's INSERT/SELECT, `add_delete_policies.sql` for its DELETE). A blocked object delete returns **200 with an empty list**, not an error, so `delete_specimen_photos()` counts the objects the bucket sends back and reports the rest as `photos_orphaned`. A blocked *upload* is the exception that does raise — 403 "new row violates row-level security policy" — which is what an upload arriving as `anon` gets. That is why `utils/auth.py::_apply_storage_token()` writes to storage3's `_headers` and not the httpx session's: storage3 passes `_headers` to httpx explicitly per request, and an explicit header beats a session default, so setting the session alone is silently ignored.
