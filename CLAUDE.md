...
Field observations are stored with a `field_screening_result` JSONB column tagged by `screening_method` (`manual_field_log`, `ai_vision`, `manual_checklist`, `trained_classifier`). Two canonical helpers in `data_manager.py` interpret that column — reuse them rather than re-parsing the JSON per page: `extract_genus_counts_from_screening()` returns `{genus: count}` (the only correct path for `manual_field_log` raw counts), and `extract_primary_genus()` returns the single resolved genus for identification methods (`None` for field logs). Loading goes through the cached `load_specimen_records()` (`@st.cache_data(ttl=60)`); **call the matching `clear_*_cache()` after any write.** `utils/pcr_and_accuracy.py` and `utils/vision_inference.py` still carry their own method-specific genus logic (kept separate for citation/complex handling) — a future consolidation target.
